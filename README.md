...
The fully sender/receiver-compatible v2 branch of `ASRT` is currently under active development! Please see [Senders/Receivers](#sendersreceivers) section for a closer look on the story behind it all, the features we plan on supporting and all the cool things you will be able to do with it!
...
* [Senders/Receivers](#sendersreceivers)
...
## Memory Management
When we have to dynamically allocate we don't want to always go to `new` and `delete` for memory management. We can leverage custom resources/allocation strategies such as stack allocation, memory pooling or malloc alternatives like `tcmalloc` and `jemalloc`. To achieve this we can either leverage exisiting memory resources from the `std::pmr` namesapce or create our own resources.

 Most allocating objects in `ASRT` like `asrt::io_context` and `asrt::Task` take an optional
`std::pmr::memory_resource` parameter which they use to allocate memory. Additionaly, certain APIs may also allow you to pass a resource parameter to manage resource allocation specifically for that operation.

For example, `asrt::io_context` is defined as
```c++
template <typename MemoryResource>
class io_context{
    explicit io_context(MemoryResource& res) {
		//...
	}
};
```

A concrete memory resource such as `std::pmr::unsynchronized_pool_resource` must be determined at compile time for use with `asrt::io_context` construction. The memory resource used defaults to `std::pmr::default_memory_resource()`. This memory resource will be used, for eg., to allocate space for individual operations.

```c++
io_context<> ctx_default; // Uses the default resource

std::pmr::monotonic_buffer_resource resource; //use stack buffer for allocation
io_context ctx(&resource);

// Post handlers
ctx.post([] { /* handler code */ });
ctx.run();
```
...
```c++
std::pmr::unsynchronized_pool_resource resource1; //uses pool resource by default
io_context ctx(&resource);

std::pmr::monotonic_buffer_resource resource2; //
ctx.Post([]{}, resource2); // use stack buffer for this operation
```
...
### Why the change?
The landscape of C++ asyncrhony is evolving. Historically, C++ has offered a variety of mechanisms for asynchronous programming, ranging from low-level thread management to higher-level abstractions like std::future and callback-based APIs. While these tools have been useful, they each come with their own set of trade-offs, and none of them has provided a truly comprehensive solution. The need for a more robust, intuitive, and unified approach to asynchrony in C++ has led to the development of the sender-receiver model. This model, detailed in the [P2300 proposal](https://www.open-std.org/jtc1/sc22/wg21/docs/papers/2024/p2300r10.html#example-hello-world), offers a powerful abstraction that unifies the various approaches to asynchronous programming into a single, cohesive framework. 

### Limitations of callbacks
In the traditional callback-based approach, composing multiple operations often leads to tangled logic, making it hard to maintain and extend. This is sometimes referred to as “callback hell,” where the flow of control becomes difficult to trace, and error handling is scattered across different callbacks.

Callback Example
```c++
io_context ctx;

socket.async_receive(buffer, [&](error_code ec, std::size_t bytes_transferred) {
    if (!ec) {
        process_data_async(buffer, bytes_transferred, [buffer](std::size_t bytes_processed){
			send_processed_data(buffer, bytes_processed);
		});
    } else {
        handle_error(ec);
    }
...
Compare with
```c++
auto receive_sender = socket.async_receive(buffer)
    | then([](std::size_t bytes_transferred) {
        return process_data_async(buffer, bytes_transferred);
    })
	| then([buffer](std::size_t bytes_processed){
		send_processed_data(buffer, bytes_processed);
	})
...
ctx.run(receive_sender); //hypothetical api
```

See how the operation chain is broken down into modular, reusable components that are easy to extend or modify. This model lets you build complex asynchronous workflows in a structured and predictable way, something that is far more challenging with callbacks.

###  `ASRT` V2 features

#### Sender-Receiver Model
 - Core Concepts: Implements the core concepts of the sender-receiver model, including sender, receiver, and operation_state.

 - Tag-Invocable Customization Points: CPOs for operations like connect, start, set_value, set_error, and set_stopped. 

#### Execution Contexts and Schedulers
 - Execution Contexts: These contexts will manage the lifetimes of threads and control where and how tasks are executed. Implemented schedulers include `static_thread_pool`, `io_context`. 

 - Schedulers: Schedulers provide the ability to schedule tasks for execution. Implemented schedulers include `run_loop`, `synchronous_blocking_scheduler`, `polling_scheduler`.

#### Sender Algorithms
- Basic Algorithms: Implements basic sender algorithms like just(), then(), when_all(), and on(). These algorithms will allow you to compose asynchronous operations in a declarative manner.

- Customization Points for Algorithms: 
Things like `execution::then`, `execution::set_value` are CPO objects rather than namespace functions.

#### Sender-returning APIs
- Previous callback-based async apis such as `asrt::ReceiveAsync` now return senders. You can attach callbacks(continuations) using sender algorithms like `execution::then`.

#### Error Handling and Cancellation
- Error Handling: this will be familiar as errors will still be able to be handled in continuation functions like before. We disabled the `execution::set_error` channel since exceptions are not used throughout the library.

- Cancellation: graceful stopping of operations using `std::stop_token` and `execution::set_stopeed`.
...
To build the examples, in root dir:

> cmake ./build

The example binaries are found in build/bin